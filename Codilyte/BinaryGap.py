...
10100 and contains one binary gap of length 1.
...
the function should return 0, because N has binary representation
...
def Solution(N):
    ...
    for i in range(len(binary)):
        if binary[i] == "1":
            previusOne=nextOne
            nextOne = i
            if nextOne-previusOne >= max:
                max = nextOne-previusOne
